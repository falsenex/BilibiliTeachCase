...
def get_password(mingwen):
    ...
    """
    # 从Base64解码密钥
    key = base64.b64decode("cXJjb2RlX3BAc3N3MHJkSw==").decode('utf-8')
    # 初始化向量
    iv = b'0102030405060708'
    # 将明文转换为字节
    plaintext_bytes = mingwen.encode('utf-8')
    # 创建AES加密器，使用CBC模式和指定的IV
...
def get_sign(post_data_get_ran_key, ran_key,type):
    if type == 'verify':
        # 定义键的顺序
        ordered_keys = [
        ...
        ordered_keys = [
            "autokey", "check_code", "check_code_type", "ct", "password", "permanent_id",
            "spermanent_id", "requestId", "t", "token",  "username"
        ]
    else:
        raise 'type error'
    post_data = "&".join(
...
def get_sliding_verify_code(request_id, ran_key):
    ...
    sign = get_sign(data, ran_key,'verify')
    data.update({
        "sign": sign
    })
...
def login(request_id,username,password,token,check_code,ran_key):
    ...
    data = {
        ...
        "check_code_type": "1",
        # "sign": "m65m+UHKnuJmWoVz2tRGWw50cmdRo/2IhuJ7dtR/UZ0cYTDrrG9Bsy/41AQ0TDdP"
    }
    sign = get_sign(data, ran_key,'login')
    data.update({
        "sign": sign
    })
    logger.warning(data)
    cookies = {
        "__permanent_id": permanent_id,
        # "JSESSIONID": "5843A1202BA192BA34A9D0550ECFDE12"
    }
...
def main():
    ...
    check_code = check_captcha(request_id, ran_key, img_data)['data']['checkCode']
    username = '151405454349'
    password_mingwen = 'xyh123456'
    password = get_password(password_mingwen)
    # password = execjs.compile(open('test.js',  'r',encoding='utf-8').read()).call('get_passwd', password_mingwen)
    login(request_id,username,password,img_data['verifyToken'],check_code,ran_key)
